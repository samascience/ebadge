...
final class ServiceMethod<R, T> {
    ...
    private final eq.a callFactory;
    private final fi1 contentType;
    private final boolean hasBody;
    private final iw0 headers;
    private final String httpMethod;
    private final boolean isFormEncoded;
    private final boolean isMultipart;
    private final ParameterHandler<?>[] parameterHandlers;
    private final String relativeUrl;
    private final Converter<fh2, R> responseConverter;
    ...
    static final class Builder<T, R> {
        ...
        Converter<fh2, T> responseConverter;
        Type responseType;
        ...
        private Converter<fh2, T> createResponseConverter() {
            try {
        ...
        private iw0 parseHeaders(String[] strArr) {
        ...
        private ParameterHandler<?> parseParameterAnnotation(int i, Type type, Annotation[] annotationArr, Annotation annotation) {
            if (annotation instanceof Url) {
                ...
                this.gotUrl = true;
                if (type == tx0.class || type == String.class || type == URI.class || ((type instanceof Class) && "android.net.Uri".equals(((Class) type).getName()))) {
            ...
            if (!(annotation instanceof Part)) {
                if (!(annotation instanceof PartMap)) {
                    ...
                    try {
                        Converter<T, ff2> converterRequestBodyConverter = this.retrofit.requestBodyConverter(type, annotationArr, this.methodAnnotations);
                        this.gotBody = true;
                ...
                if (String.class == parameterUpperBound4) {
                    ...
                    if (gm1.c.class.isAssignableFrom(Utils.getRawType(parameterUpperBound5))) {
            ...
            if (strValue5.isEmpty()) {
                if (!Iterable.class.isAssignableFrom(rawType9)) {
                    if (rawType9.isArray()) {
                        if (gm1.c.class.isAssignableFrom(rawType9.getComponentType())) {
                            return ParameterHandler.RawPart.INSTANCE.array();
                        }
                        throw parameterError(i, "@Part annotation must supply a name or use MultipartBody.Part parameter type.", new Object[0]);
                    }
                    if (gm1.c.class.isAssignableFrom(rawType9)) {
                        return ParameterHandler.RawPart.INSTANCE;
                    }
                    throw parameterError(i, "@Part annotation must supply a name or use MultipartBody.Part parameter type.", new Object[0]);
                }
                if (type instanceof ParameterizedType) {
                    if (gm1.c.class.isAssignableFrom(Utils.getRawType(Utils.getParameterUpperBound(0, (ParameterizedType) type)))) {
            ...
            iw0 iw0VarE = iw0.e("Content-Disposition", "form-data; name=\"" + strValue5 + "\"", "Content-Transfer-Encoding", part.encoding());
            if (!Iterable.class.isAssignableFrom(rawType9)) {
                if (!rawType9.isArray()) {
                    if (gm1.c.class.isAssignableFrom(rawType9)) {
                    ...
                    return new ParameterHandler.Part(iw0VarE, this.retrofit.requestBodyConverter(type, annotationArr, this.methodAnnotations));
                }
                Class<?> clsBoxIfPrimitive = ServiceMethod.boxIfPrimitive(rawType9.getComponentType());
                if (gm1.c.class.isAssignableFrom(clsBoxIfPrimitive)) {
                    throw parameterError(i, "@Part parameters using the MultipartBody.Part must not include a part name in the annotation.", new Object[0]);
                }
                return new ParameterHandler.Part(iw0VarE, this.retrofit.requestBodyConverter(clsBoxIfPrimitive, annotationArr, this.methodAnnotations)).array();
            }
            if (type instanceof ParameterizedType) {
                Type parameterUpperBound6 = Utils.getParameterUpperBound(0, (ParameterizedType) type);
                if (gm1.c.class.isAssignableFrom(Utils.getRawType(parameterUpperBound6))) {
                    throw parameterError(i, "@Part parameters using the MultipartBody.Part must not include a part name in the annotation.", new Object[0]);
                }
                return new ParameterHandler.Part(iw0VarE, this.retrofit.requestBodyConverter(parameterUpperBound6, annotationArr, this.methodAnnotations)).iterable();
            }
            throw parameterError(i, rawType9.getSimpleName() + " must include generic type (e.g., " + rawType9.getSimpleName() + "<String>)", new Object[0]);
        }
        ...
        public ServiceMethod build() {
            ...
            if (typeResponseType == Response.class || typeResponseType == eh2.class) {
        ...
        private RuntimeException methodError(Throwable th, String str, Object... objArr) {
            return new IllegalArgumentException(String.format(str, objArr) + "\n    for method " + this.method.getDeclaringClass().getSimpleName() + FileUtils.FILE_EXTENSION_SEPARATOR + this.method.getName(), th);
        }
    ...
    eq toCall(@Nullable Object... objArr) throws IOException {
        ...
        if (length == parameterHandlerArr.length) {
            ...
            return this.callFactory.a(requestBuilder.build());
        }
        throw new IllegalArgumentException("Argument count (" + length + ") doesn't match expected count (" + parameterHandlerArr.length + ")");
    }

    R toResponse(fh2 fh2Var) throws IOException {
        return this.responseConverter.convert(fh2Var);
    }
}
