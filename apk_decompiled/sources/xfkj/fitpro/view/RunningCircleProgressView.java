...
public class RunningCircleProgressView extends View {
    ...
    private void a(Canvas canvas) {
        ...
        if (this.J) {
            ...
            int i = (int) ((this.G / 100.0f) * this.f421q);
            if (this.K) {
                for (int i2 = 0; i2 < this.f421q; i2++) {
        ...
        float f4 = this.J ? this.m - ((this.n + this.c) * 2.0f) : this.m;
        float f5 = f4 * 2.0f;
        float f6 = this.f - f4;
        float f7 = this.g - f4;
        float f8 = f6 + f5;
        ...
        if (this.T) {
            ...
            double height = ((double) (getHeight() / 2)) + (Math.sin(d2) * d);
            if (this.S) {
                this.N.setShadowLayer(10.0f, 0.0f, 0.0f, -7829368);
            }
            float f11 = (float) width;
            float f12 = (float) height;
            canvas.drawCircle(f11, f12, this.Q / 2.0f, this.N);
            Bitmap bitmap = this.P;
            if (bitmap != null && this.O) {
                int width2 = bitmap.getWidth() > this.P.getHeight() ? this.P.getWidth() : this.P.getHeight();
                float f13 = (this.Q / 10.0f) * 9.0f;
                float f14 = width2;
                if (f14 > f13) {
                    float f15 = f13 / f14;
                    this.P = ImageUtils.c(this.P, f15, f15);
                }
                this.N.clearShadowLayer();
                Bitmap bitmap2 = this.P;
                canvas.drawBitmap(bitmap2, f11 - (bitmap2.getWidth() / 2), f12 - (this.P.getHeight() / 2), this.N);
            }
            double ratio = (((double) ((this.e * getRatio()) - 90.0f)) * 3.141592653589793d) / 180.0d;
            double width3 = ((double) (getWidth() / 2)) + (Math.cos(ratio) * d);
            double height2 = ((double) (getHeight() / 2)) + (d * Math.sin(ratio));
            float f16 = (float) width3;
            float f17 = (float) height2;
            canvas.drawCircle(f16, f17, this.Q / 2.0f, this.N);
            Bitmap bitmap3 = this.P;
            if (bitmap3 == null || !this.O) {
                return;
            }
            int width4 = bitmap3.getWidth() > this.P.getHeight() ? this.P.getWidth() : this.P.getHeight();
            float f18 = (this.Q / 10.0f) * 9.0f;
            float f19 = width4;
            if (f19 > f18) {
                float f20 = f18 / f19;
                this.P = ImageUtils.c(this.P, f20, f20);
            }
            this.N.clearShadowLayer();
            Bitmap bitmap4 = this.P;
            canvas.drawBitmap(bitmap4, f16 - (bitmap4.getWidth() / 2), f17 - (this.P.getHeight() / 2), this.N);
        }
    }
    ...
    private void c(Context context, AttributeSet attributeSet) {
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(attributeSet, R$styleable.RunningCircleProgressView);
        DisplayMetrics displayMetrics = getDisplayMetrics();
        float f = 12.0f;
        float fApplyDimension = TypedValue.applyDimension(1, 12.0f, displayMetrics);
        this.c = fApplyDimension;
        this.V = fApplyDimension * 2.0f;
        int i = 2;
        ...
        while (i2 < indexCount) {
            ...
            if (index == 20) {
                this.c = typedArrayObtainStyledAttributes.getDimension(index, TypedValue.applyDimension(1, f, displayMetrics));
            } else if (index == 17) {
                this.V = typedArrayObtainStyledAttributes.getDimension(index, TypedValue.applyDimension(1, f, displayMetrics));
            } else if (index == 16) {
                ...
                this.v = typedArrayObtainStyledAttributes.getDimension(index, TypedValue.applyDimension(i, 30.0f, displayMetrics));
            } else if (index == 13) {
                ...
                this.Q = typedArrayObtainStyledAttributes.getDimension(index, f);
            } else if (index == 5) {
                ...
                this.U = typedArrayObtainStyledAttributes.getDimension(index, f);
            } else if (index == 21) {
        ...
        this.f421q = (int) ((this.e * 1.0f) / (this.o + this.p));
