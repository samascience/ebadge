...
public class NfcMsg {
    ...
    public static NfcMsg parseNfcMsg(byte[] bArr) {
        ...
        int iBytesToInt = CHexConver.bytesToInt(bArr, 42, 2);
        JL_Log.e("NfcMsg", "parseNfcMsg", "nfcHeadLen = " + iBytesToInt + ", data.len = " + bArr.length);
        if (bArr.length != iBytesToInt + 44) {
            return null;
        }
        ...
        long jIntToTime = RcspUtil.intToTime(byteBufferWrap.getInt());
        ...
        if (s2 > 2) {
            s3 = byteBufferWrap.getShort();
            bArr5 = new byte[s2 - 2];
            byteBufferWrap.get(bArr5);
        }
        return new NfcMsg().setId(s).setReserve(bArr2).setDevHandler(i).setUpdateTime(jIntToTime).setNfcFileName(str).setNickname(strTrim).setNfcHeadLen(s2).setCrc16(s3).setNfcID(bArr5);
    }
