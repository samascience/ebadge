...
public class BluetoothManager implements IBluetoothOperation {
    ...
    private final Handler mHandler = new Handler(Looper.getMainLooper(), new Handler.Callback() { // from class: ml
    ...
    private class RemoveHistoryRecordTask implements Runnable {
        ...
        public RemoveHistoryRecordTask(HistoryRecord historyRecord, OnHistoryRecordCallback onHistoryRecordCallback) {
        ...
        public void run() {
            ...
            int i = this.state;
            if (i == 0) {
                remoteDevice = BluetoothUtil.getRemoteDevice(BluetoothManager.this.mContext, this.mHistoryRecord.getAddress());
            } else if (i != 1) {
                remoteDevice = null;
            } else if (this.mHistoryRecord.getAddress().equals(this.mHistoryRecord.getMappedAddress())) {
                JL_Log.d(BluetoothManager.TAG, this.tag, "same address, skip");
                this.state++;
                remoteDevice = null;
            } else {
                remoteDevice = BluetoothUtil.getRemoteDevice(BluetoothManager.this.mContext, this.mHistoryRecord.getMappedAddress());
            }
            JL_Log.d(BluetoothManager.TAG, this.tag, "start ---> state = " + this.state + ", device = " + BluetoothManager.this.printDeviceInfo(remoteDevice));
            if (remoteDevice != null) {
                this.removeAddress = remoteDevice.getAddress();
                if (BluetoothManager.this.isConnectedDevice(remoteDevice)) {
                    this.disconnectAddress = remoteDevice.getAddress();
                    BluetoothManager.this.mHandler.postDelayed(this, ProtocolConstants.CONNECTION_TIMEOUT_MS);
                    BluetoothManager.this.disconnectBtDevice(remoteDevice);
                    JL_Log.i(BluetoothManager.TAG, this.tag, "disconnectBtDevice >>> ");
                    return;
                }
                if (BluetoothManager.this.isConnectedByProfile(remoteDevice) == 2) {
                    this.disconnectAddress = remoteDevice.getAddress();
                    BluetoothManager.this.mHandler.postDelayed(this, ProtocolConstants.CONNECTION_TIMEOUT_MS);
                    if (BluetoothManager.this.disconnectByProfiles(remoteDevice)) {
                        JL_Log.i(BluetoothManager.TAG, this.tag, "disconnectByProfiles >>> ");
                        return;
                    }
                }
                this.disconnectAddress = null;
                if (BluetoothManager.this.isPaired(remoteDevice)) {
                    boolean zTryToUnPair = BluetoothManager.this.tryToUnPair(remoteDevice);
                    JL_Log.i(BluetoothManager.TAG, this.tag, "tryToUnPair ---> " + zTryToUnPair);
                    if (zTryToUnPair) {
                        this.state++;
    ...
    public BluetoothManager(Context context, BluetoothOption bluetoothOption) {
        OnBtDiscoveryListener onBtDiscoveryListener = new OnBtDiscoveryListener() { // from class: com.jieli.bluetooth_connect.impl.BluetoothManager.2
            ...
            public void onDiscoveryError(ErrorInfo errorInfo) {
                JL_Log.e(BluetoothManager.TAG, "onDiscoveryError", Constants.STR_EMPTY + errorInfo);
        ...
        OnBtDevicePairListener onBtDevicePairListener = new OnBtDevicePairListener() { // from class: com.jieli.bluetooth_connect.impl.BluetoothManager.3
            ...
            public void onBtDeviceBond(BluetoothDevice bluetoothDevice, int i) {
                ...
                if (BluetoothManager.this.mBluetoothOption.isNotAssociatedEDR()) {
                    JL_Log.w(BluetoothManager.TAG, "onBtDeviceBond", ConnectUtil.formatString("Device[%s] is un-bonded.Skipping the progress of delete history record.", bluetoothDevice));
                    return;
                }
        ...
        OnBrEdrListener onBrEdrListener = new OnBrEdrListener() { // from class: com.jieli.bluetooth_connect.impl.BluetoothManager.4
            ...
            public void onBrEdrConnection(BluetoothDevice bluetoothDevice, int i) {
                ...
                if (i != 2) {
                    if (i == 0) {
                        BluetoothManager.this.notifyRemoveHistoryRecordTask(bluetoothDevice, 1);
                        return;
                    }
                    return;
                }
                HistoryRecord historyRecord = BluetoothManager.this.getHistoryRecord(bluetoothDevice.getAddress());
                if (historyRecord == null) {
                    return;
                }
                BluetoothDevice remoteDevice = BluetoothUtil.getRemoteDevice(BluetoothManager.this.mContext, historyRecord.getAddress());
                boolean zIsConnectedDevice = BluetoothManager.this.isConnectedDevice(remoteDevice);
                boolean z = !zIsConnectedDevice;
                if (!zIsConnectedDevice && !BluetoothManager.this.mBluetoothOption.isUseMultiDevice() && BluetoothManager.this.getConnectedDevice() != null) {
                    BluetoothManager bluetoothManager = BluetoothManager.this;
        ...
        OnBtSppListener onBtSppListener = new OnBtSppListener() { // from class: com.jieli.bluetooth_connect.impl.BluetoothManager.5
            ...
            public void onSppConnection(BluetoothDevice bluetoothDevice, UUID uuid, int i) {
                ...
                JL_Log.i(BluetoothManager.TAG, "onSppConnection", "device : " + BluetoothManager.this.printDeviceInfo(bluetoothDevice) + ", uuid = " + uuid + ", state : " + BluetoothConstant.printBtConnection(i));
                if (i != 1) {
        ...
        OnBtBleListener onBtBleListener = new OnBtBleListener() { // from class: com.jieli.bluetooth_connect.impl.BluetoothManager.6
            ...
            public void onBleConnection(BluetoothDevice bluetoothDevice, int i) {
                ...
                JL_Log.i(BluetoothManager.TAG, "onBleConnection", "device : " + BluetoothManager.this.printDeviceInfo(bluetoothDevice) + ", status : " + BluetoothConstant.printBtConnection(i));
                if (i != 1) {
                    BluetoothManager.this.notifyConnectionStatus(bluetoothDevice, i);
                }
            }
        ...
        JL_Log.i(TAG, "init", "Lib name = " + BluetoothConstant.getLibVersionName() + ", option = " + bluetoothOption + ",\n clazz : " + this);
        ...
        bluetoothPair.setBluetoothOption(bluetoothOption);
        ...
        bluetoothBrEdr.setBluetoothOption(bluetoothOption);
    ...
    private void changeConnectedDevice(BluetoothDevice bluetoothDevice) {
        ...
        JL_Log.d(TAG, "changeConnectedDevice", "device : " + printDeviceInfo(bluetoothDevice));
    }
    ...
    public void notifyConnectionStatus(BluetoothDevice bluetoothDevice, int i) {
        HistoryRecord historyRecord;
        boolean zCheckIsValidDevice = checkIsValidDevice(bluetoothDevice);
        String str = TAG;
        JL_Log.e(str, "notifyConnectionStatus", "device : " + printDeviceInfo(bluetoothDevice) + ", checkIsValidDevice : " + zCheckIsValidDevice + ", status : " + BluetoothConstant.printBtConnection(i));
        if (zCheckIsValidDevice) {
            if (i != 1) {
                ...
                if (i == 0) {
                    ...
                    JL_Log.i(str, "notifyConnectionStatus", "CONNECTION_OK, connected List : " + getConnectedDeviceList().size() + ", connectWay : " + (zIsConnectedSppDevice ? 1 : 0));
                    if (!zIsConnectedSppDevice && getBluetoothOption().isNeedChangeBleMtu() && getBluetoothOption().getMtu() > 20 && this.mBluetoothBle.requestBleMtu(bluetoothDevice, getBluetoothOption().getMtu())) {
    ...
    public boolean notifyRemoveHistoryRecordTask(BluetoothDevice bluetoothDevice, int i) {
        ...
        if (this.mRemoveHistoryRecordTask != null && bluetoothDevice != null) {
            ...
            if (z) {
                JL_Log.i(TAG, "notifyRemoveHistoryRecordTask", "device = " + bluetoothDevice + ", way = " + i);
        ...
        if (ConnectUtil.isHasConnectPermission(this.mContext) && (systemConnectedBtDeviceList = BluetoothUtil.getSystemConnectedBtDeviceList(this.mContext)) != null) {
            ...
            for (BluetoothDevice bluetoothDevice : systemConnectedBtDeviceList) {
                if (!isConnectedDevice(bluetoothDevice) && !getDiscoveredBluetoothDevices().contains(bluetoothDevice)) {
                    ...
                    if ((z && (type == 2 || type == 3)) || (bluetoothDevice.getName() != null && scanFilterData != null && bluetoothDevice.getName().startsWith(scanFilterData))) {
    ...
    public void destroy() {
        JL_Log.w(TAG, "destroy", Constants.STR_EMPTY);
    ...
    public boolean isConnectedDevice(BluetoothDevice bluetoothDevice) {
        boolean zIsConnectedBleDevice = this.mBluetoothBle.isConnectedBleDevice(bluetoothDevice);
        boolean zIsConnectedSppDevice = this.mBluetoothSpp.isConnectedSppDevice(bluetoothDevice);
        JL_Log.d(TAG, "isConnectedDevice", "device : " + bluetoothDevice + ", isBleConnected = " + zIsConnectedBleDevice + ", isSppConnected = " + zIsConnectedSppDevice);
        return zIsConnectedBleDevice || zIsConnectedSppDevice;
    }
    ...
    public boolean isMatchDevice(BluetoothDevice bluetoothDevice, BluetoothDevice bluetoothDevice2) {
        ...
        boolean zDeviceEquals = BluetoothUtil.deviceEquals(bluetoothDevice, bluetoothDevice2);
        if (zDeviceEquals) {
            return zDeviceEquals;
        }
    ...
    public boolean registerBluetoothCallback(BluetoothEventCallback bluetoothEventCallback) {
        return this.mEventCbManager.addListener(bluetoothEventCallback);
    }
    ...
    public void setBluetoothOption(BluetoothOption bluetoothOption) {
        ...
        this.mBluetoothOption = bluetoothOption;
        JL_Log.d(TAG, "setBluetoothOption", Constants.STR_EMPTY + bluetoothOption);
    ...
    public boolean unregisterBluetoothCallback(BluetoothEventCallback bluetoothEventCallback) {
        return this.mEventCbManager.removeListener(bluetoothEventCallback);
    }

    @Override // com.jieli.bluetooth_connect.interfaces.IBluetoothOperation
    public boolean writeDataToBLEDevice(BluetoothDevice bluetoothDevice, UUID uuid, UUID uuid2, byte[] bArr) {
        boolean zIsConnectedBLEDevice = isConnectedBLEDevice(bluetoothDevice);
        if (zIsConnectedBLEDevice) {
            this.mBluetoothBle.writeDataByBleAsync(bluetoothDevice, uuid, uuid2, bArr, new OnWriteDataCallback() { // from class: ll
            ...
            JL_Log.d(TAG, "writeDataToBLEDevice", "device(" + bluetoothDevice + ") is disconnected.");
        }
        return zIsConnectedBLEDevice;
    }
