    ...
    public CommandBase parseDataToCmd(BasePacket basePacket, CommandBase commandBase) {
        ...
        if (basePacket.getType() == 1) {
            if (paramData != null && paramData.length >= 2) {
                iBytesToInt = CHexConver.bytesToInt(paramData[0], paramData[1]);
            }
            return new SettingsMtuCmd(new SettingsMtuParam(iBytesToInt)).setOpCodeSn(basePacket.getOpCodeSn());
        }
