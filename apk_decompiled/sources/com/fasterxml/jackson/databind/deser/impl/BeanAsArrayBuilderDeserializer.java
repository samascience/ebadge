...
public class BeanAsArrayBuilderDeserializer extends BeanDeserializerBase {
    ...
    protected Object _deserializeFromNonArray(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException {
        return deserializationContext.handleUnexpectedToken(getValueType(deserializationContext), jsonParser.D(), jsonParser, "Cannot deserialize a POJO (of type %s) from non-Array representation (token: %s): type/property designed to be serialized as JSON Array", this._beanType.getRawClass().getName(), jsonParser.D());
    }
            ...
            if (settableBeanProperty == null) {
                ...
                if (!bVarG.i(name) || settableBeanPropertyE != null) {
                    if (settableBeanPropertyE == null) {
                        ...
                        try {
                            objDeserializeSetAndReturn = propertyBasedCreator.a(deserializationContext, bVarG);
                            if (objDeserializeSetAndReturn.getClass() != this._beanType.getRawClass()) {
                                JavaType javaType = this._beanType;
                                return deserializationContext.reportBadDefinition(javaType, String.format("Cannot support implicit polymorphic deserialization for POJOs-as-Arrays style: nominal type %s, actual type %s", ay.G(javaType), objDeserializeSetAndReturn.getClass().getName()));
                            }
                        } catch (Exception e2) {
