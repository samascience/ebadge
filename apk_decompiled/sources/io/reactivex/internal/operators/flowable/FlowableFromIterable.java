...
public final class FlowableFromIterable<T> extends Flowable<T> {
    ...
    static final class IteratorSubscription<T> extends BaseRangeSubscription<T> {
        ...
        IteratorSubscription(cw2 cw2Var, Iterator<? extends T> it) {
            super(it);
        ...
        void fastPath() {
            ...
            while (!this.cancelled) {
                try {
                    ...
                    if (next == null) {
                        cw2Var.onError(new NullPointerException("Iterator.next() returned a null value"));
                        return;
                    }
                    cw2Var.onNext(next);
                    if (this.cancelled) {
    ...
    public static <T> void subscribe(cw2 cw2Var, Iterator<? extends T> it) {
        try {
            if (!it.hasNext()) {
                EmptySubscription.complete(cw2Var);
            } else if (cw2Var instanceof ConditionalSubscriber) {
                cw2Var.onSubscribe(new IteratorConditionalSubscription((ConditionalSubscriber) cw2Var, it));
            } else {
                cw2Var.onSubscribe(new IteratorSubscription(cw2Var, it));
            }
        } catch (Throwable th) {
            Exceptions.throwIfFatal(th);
    ...
    public void subscribeActual(cw2 cw2Var) {
        try {
            subscribe(cw2Var, this.source.iterator());
        } catch (Throwable th) {
            Exceptions.throwIfFatal(th);
            EmptySubscription.error(th, cw2Var);
        }
